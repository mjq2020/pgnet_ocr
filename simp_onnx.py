...
def simplify_onnx(input_path, output_path, check_model=True, check_n=3, 
                  perform_optimization=True, skip_fuse_bn=False,
                  input_shapes=None, dynamic_input_shape=False):
    """
    简化ONNX模型
    ...
    try:
        ...
        if dynamic_input_shape:
            print("警告: --dynamic-input-shape 参数已被废弃，onnxsim现在原生支持动态输入形状")
...
def main():
    ...
    parser.add_argument("--check-n", type=int, default=3, help="一致性检查的随机样本数量")
    parser.add_argument("--no-optimization", action="store_true", help="跳过优化步骤")
    parser.add_argument("--skip-fuse-bn", action="store_true", help="跳过BatchNormalization融合")
    parser.add_argument("--dynamic-input-shape", action="store_true", 
